...
import { MessageCircle, Package, Clock, AlertTriangle, TrendingUp, TrendingDown, ArrowUpRight, AlertCircle, Info, CheckCircle2 } from "lucide-react";
import { Badge } from "@/components/shared/Badge";
import { MessagesByHourChart, OrdersWeekLineChart } from "./Charts";
import { messagesByHour, ordersThisWeek, agentPerformance, recentFlags } from "@/data/mockAnalytics";
...
function MetricCard({ icon: Icon, label, value, delta, deltaUp, sub }: { icon: any; label: string; value: string; delta?: string; deltaUp?: boolean; sub?: string }) {
  return (
    <div className="bg-card rounded-xl border border-border p-5 hover:shadow-sm transition-shadow">
      ...
      {delta && (
        <div className={cn("mt-3 flex items-center gap-1 text-xs font-semibold", deltaUp ? "text-success" : "text-success")}>
  ...
  return (
    <div className="h-full overflow-y-auto">
      <div className="p-6 max-w-[1600px] mx-auto space-y-5">
        <div>
          <p className="text-sm text-muted-foreground">Welcome back, Sarah · <span className="text-foreground font-medium">{business}</span></p>
          <h2 className="text-2xl font-bold text-foreground mt-0.5">Today's Overview</h2>
        </div>

        <div className="grid grid-cols-4 gap-4">
          <MetricCard icon={MessageCircle} label="Total Messages Today" value="47" delta="↑ 12%" deltaUp />
          ...
          <div className="bg-card rounded-xl border border-border p-5">
            ...
            <ul className="space-y-2">
              {recentFlags.map((f, i) => (
                <li key={i}>
                  <button
                    onClick={() => { setActiveChatId(f.customerId); router.navigate({ to: "/messages" }); }}
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-card rounded-xl border border-border p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => router.navigate({ to: "/meetings" })} className="text-xs text-primary hover:underline font-medium">View all</button>
            </div>
            <ul className="space-y-2.5">
              {upcoming.map((m) => {
                const cust = mockCustomers.find((c) => c.id === m.customerId);
                return (
                  <li key={m.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-accent/50">
                    <div className="h-9 w-9 rounded-full text-white font-bold text-xs flex items-center justify-center" style={{ background: cust?.avatarColor }}>
                      {cust?.initials}
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      <Badge variant={m.status === "Scheduled" ? "primary" : "warning"} className="mt-0.5">{m.status}</Badge>
                    </div>
                  </li>
                );
              })}
          ...
          <div className="bg-card rounded-xl border border-border p-5">
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => router.navigate({ to: "/catalog" })} className="text-xs text-primary hover:underline font-medium">View catalog</button>
            </div>
            <ul className="space-y-2.5">
              {lowStock.map((p) => (
                <li key={p.id} className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-accent/50">
                  <div className="h-9 w-9 rounded-md text-white font-bold text-sm flex items-center justify-center" style={{ background: p.color }}>{p.initial}</div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-foreground truncate">{p.name}</div>
                    <div className="text-xs text-muted-foreground">SKU {p.sku} · RM{p.price}</div>
                  </div>
