...
export function Sidebar() {
  ...
  return (
    ...
    >
      <div className={cn("h-14 flex items-center border-b border-sidebar-border px-3 gap-2", sidebarCollapsed && "justify-center px-0")}>
        <div className="h-8 w-8 rounded-md bg-primary flex items-center justify-center text-primary-foreground font-bold text-sm shrink-0">B</div>
        {!sidebarCollapsed && <span className="font-bold text-sidebar-foreground">BizAssist <span className="text-primary">AI</span></span>}
      </div>
      ...
      <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
        {NAV.map((item) => {
          ...
          const badge = item.badge === "messages" ? totalUnread : item.badge === "orders" ? pendingOrders : 0;
          return (
            <Link
              key={item.to}
              to={item.to}
      ...
      <div className="border-t border-sidebar-border p-2 space-y-2">
        {!sidebarCollapsed ? (
          <div>
            <label className="text-[10px] uppercase tracking-wider text-muted-foreground px-2">Business</label>
            <select
              value={business}
        ...
        <div className={cn("flex items-center gap-2 rounded-md p-2 bg-card/50", sidebarCollapsed && "justify-center")}>
          <div className="h-8 w-8 rounded-full bg-primary text-primary-foreground flex items-center justify-center text-xs font-bold shrink-0">SL</div>
          {!sidebarCollapsed && (
            <div className="min-w-0">
              <div className="text-xs font-semibold text-sidebar-foreground truncate">Sarah Lim</div>
