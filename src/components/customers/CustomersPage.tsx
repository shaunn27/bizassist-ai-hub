  ...
  return (
    <div className="h-full overflow-y-auto p-6">
      ...
      {active && (
        ...
        >
          <div
            className="w-[440px] bg-card border-l border-border h-full overflow-y-auto"
          ...
          >
            ...
            <div className="p-4 space-y-4">
              <div className="text-xs text-muted-foreground">
                Loyal customer since {active.loyalSince}
              </div>
              <div className="bg-primary-soft border border-primary/30 rounded-md p-3">
                <div className="text-[10px] uppercase font-bold text-primary-dark dark:text-accent-foreground mb-1">
                  AI Behavior Summary
                </div>
                <p className="text-xs text-foreground">{active.behaviorSummary}</p>
              </div>
              <div className="grid grid-cols-3 gap-2 text-center">
                <div className="p-3 bg-secondary rounded-md">
                  <div className="text-lg font-bold">{active.totalOrders}</div>
                  <div className="text-[10px] uppercase text-muted-foreground">Orders</div>
                </div>
                <div className="p-3 bg-secondary rounded-md">
                  <div className="text-lg font-bold">RM{active.totalSpent}</div>
                  <div className="text-[10px] uppercase text-muted-foreground">Lifetime</div>
                </div>
                <div className="p-3 bg-secondary rounded-md">
                  <div className="text-lg font-bold">RM{active.avgOrderValue}</div>
                  <div className="text-[10px] uppercase text-muted-foreground">Avg</div>
                </div>
              </div>
              <div>
                <div className="text-xs font-semibold mb-2">Preferred products</div>
                <div className="flex flex-wrap gap-1">
                  {active.preferredProducts.map((p) => (
              ...
              <div>
                <div className="text-xs font-semibold mb-2">Order history</div>
                <ul className="space-y-1.5">
                  {active.orderHistory.map((o) => (
                    <li
                      key={o.id}
                      className="flex justify-between text-xs border border-border rounded-md p-2"
                    >
                      <div>
                        <span className="font-mono text-muted-foreground">{o.id}</span> · {o.items}
                      </div>
                      <div className="font-semibold">RM{o.amount}</div>
                    </li>
                  ))}
                </ul>
              </div>
              <div>
                <div className="text-xs font-semibold mb-1">Last conversation</div>
                <p className="text-xs text-muted-foreground italic bg-secondary p-2 rounded-md">
                  "{active.lastConversationExcerpt}"
                </p>
              </div>
            </div>
          </div>
